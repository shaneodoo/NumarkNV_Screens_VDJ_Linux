"""
Windows VDJ open/close SysEx for Numark NV dual LCDs.

Source of truth: captures/VDJ_start_stop_sequence.pcapng (2026-07-29)
and captures/extracted-start-stop/screen-sysex.tsv.

USB map (USBPcap bus.device.endpoint):
...
# Layout blob (0530) — same payload on both products in Windows capture; we send
...
# Short host→device CC packets that precede open on Windows (USB-MIDI cells).
...
def close_sequence() -> list[tuple[int, bytes]]:
    """(usb_product_id, sysex) Windows-style zero clear — Left then Right."""
    out: list[tuple[int, bytes]] = []
    for m in CLOSE_LEFT:
        out.append((PID_AUDIO, m))
...
# ---------------------------------------------------------------------------
# Full empty-deck UI + close from WinBoat VDJ_start_stop_sequence.pcapng
# ---------------------------------------------------------------------------
