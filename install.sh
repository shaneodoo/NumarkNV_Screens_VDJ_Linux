...
step 3/7 "USB identity → config/nv-ids.env (from this machine's device)…"
# Never ship another user's GUIDs/paths — only VID/PIDs from a live scan (or stock NV).
DETECT="$DEST/tools/detect-nv.sh"
if [[ -x "$DETECT" || -f "$DETECT" ]]; then
  chmod +x "$DETECT" 2>/dev/null || true
  if bash "$DETECT" --write "$DEST/config/nv-ids.env"; then
    ok "nv-ids.env written for this install"
    # shellcheck source=/dev/null
    source "$DEST/config/nv-ids.env" 2>/dev/null || true
    if [[ -n "${NV_VID:-}" ]]; then
      echo "    VID=$NV_VID  control=${NV_PID_CONTROL:-?} audio=${NV_PID_AUDIO:-?} graphics=${NV_PID_GRAPHICS:-?}"
    fi
  else
    warn "detect-nv.sh --write failed — keeping shipped stock IDs"
  fi
else
  warn "tools/detect-nv.sh missing — keeping shipped stock Numark NV IDs"
fi

# udev rules: regenerate from detected VID so a non-15e4 variant still works
RULES_SRC="$DEST/config/udev/99-numark-nv.rules"
if [[ -f "$DEST/config/nv-ids.env" ]]; then
  # shellcheck source=/dev/null
  source "$DEST/config/nv-ids.env" 2>/dev/null || true
fi
_UDEV_VID="${NV_VID:-15e4}"
mkdir -p "$DEST/config/udev"
cat >"$RULES_SRC" <<RULES
# Numark NV — user access to Control / Graphics / Audio USB interfaces
# Generated by install.sh from config/nv-ids.env (VID=${_UDEV_VID})
SUBSYSTEM=="usb", ATTR{idVendor}=="${_UDEV_VID}", MODE="0666", TAG+="uaccess"
SUBSYSTEM=="sound", ATTRS{idVendor}=="${_UDEV_VID}", MODE="0666", TAG+="uaccess"
KERNEL=="midiC*D*", ATTRS{idVendor}=="${_UDEV_VID}", MODE="0666", TAG+="uaccess"
KERNEL=="controlC*", ATTRS{idVendor}=="${_UDEV_VID}", MODE="0666", TAG+="uaccess"
KERNEL=="pcmC*D*", ATTRS{idVendor}=="${_UDEV_VID}", MODE="0666", TAG+="uaccess"
RULES
ok "udev rules template → VID=${_UDEV_VID}"

step 4/7 "Shortcuts in $BIN"
# Thin wrappers only — always run code from $DEST (no second full copy of scripts)
...
cat > "$BIN/vdj-set-nv-audio.py" <<WRAP
#!/usr/bin/env bash
# Wrapper — real script lives in the install tree (portable paths / nv-ids.env)
export ROOT="$DEST"
export WINEPREFIX="\${WINEPREFIX:-$WINEPREFIX}"
exec python3 "$DEST/bin/vdj-set-nv-audio.py" "\$@"
WRAP
# Keep .py name for docs/muscle memory; body is a shell wrapper
chmod 0755 "$BIN/vdj-set-nv-audio.py"
ok "vdj-set-nv-audio.py → $DEST/bin/vdj-set-nv-audio.py"
...
step 5/7 "DXVK video stack (deck video / karaoke under Wine)…"
...
step 6/7 "USB udev rules (optional password)…"
if [[ -f "$RULES_SRC" ]]; then
  if [[ -f /etc/udev/rules.d/99-numark-nv.rules ]]; then
    # Refresh if our VID changed
    if ! grep -q "idVendor}==\"${_UDEV_VID}\"" /etc/udev/rules.d/99-numark-nv.rules 2>/dev/null; then
      if [[ -w /etc/udev/rules.d ]] 2>/dev/null; then
        cp -a "$RULES_SRC" /etc/udev/rules.d/99-numark-nv.rules
        udevadm control --reload-rules 2>/dev/null || true
        ok "udev rules updated for VID=${_UDEV_VID}"
      elif ask_yn "Update system udev rules for VID=${_UDEV_VID}? [Y/n] " Y; then
        sudo cp -a "$RULES_SRC" /etc/udev/rules.d/99-numark-nv.rules \
          && sudo udevadm control --reload-rules \
          && sudo udevadm trigger \
          && ok "udev rules updated" \
          || warn "sudo udev update failed"
      else
        warn "System udev still has old rules — copy $RULES_SRC when ready"
      fi
    else
      ok "udev rules already present (VID=${_UDEV_VID})"
    fi
...
step 7/7 "Logo restore sudoers (optional)"
...
echo "    • USB IDs in $DEST/config/nv-ids.env (from this machine's scan)"
...
echo
echo "  Re-detect USB IDs after hardware change:"
echo "    $DEST/tools/detect-nv.sh --write"
echo
