...
/* --- Numark NV hard-push (Wine 11) ----------------------------------------- */
static int nv_skip_port(const char *client, const char *port)
{
    if (!client) client = "";
    if (!port) port = "";
    if (strstr(client, "PipeWire")) return 1;
    if (strstr(client, "Midi Through")) return 1;
    if (strstr(client, "aseqdump")) return 1;
    if (strstr(client, "qpwgraph")) return 1;
    /* bridge sinks only — not real controllers */
    if (strstr(client, "nv-screens") && !strstr(client, "facade")) return 1;
    /* Kernel USB-MIDI ports are "… MIDI 1"; facade uses clean factory names */
    if (strstr(port, "MIDI 1")) return 1;
    return 0;
}

static void nv_apply_usb_ids(const char *name, WORD *wMid, WORD *wPid)
{
    if (!name || !wMid || !wPid) return;
    if (strstr(name, "pid_1005") || strstr(name, "PID_1005") || strstr(name, "NV Control"))
    { *wMid = 0x15E4; *wPid = 0x1005; return; }
    if (strstr(name, "pid_1033") || strstr(name, "PID_1033")
        || strstr(name, "NV Display Left") || strstr(name, "NV Audio"))
    { *wMid = 0x15E4; *wPid = 0x1033; return; }
    if (strstr(name, "pid_2033") || strstr(name, "PID_2033")
        || strstr(name, "NV Display Right") || strstr(name, "NV Graphics"))
    { *wMid = 0x15E4; *wPid = 0x2033; return; }
}

/* Rewrite szPname to Windows-style USB path so VDJ "Identified by PID/VID" fires.
 * MAXPNAMELEN is 32 including NUL → keep <= 31 printable chars.
 *   \\?\usb#vid_15e4&pid_1033&mi_00  = 28 chars
 */
static void nv_rewrite_usb_path(char *name, size_t name_size)
{
    const char *path = NULL;
    if (!name) return;
    if (strstr(name, "NV Display Left") || strstr(name, "pid_1033") ||
        (strstr(name, "NV Audio") && !strstr(name, "MIDI")))
        path = "\\\\?\\usb#vid_15e4&pid_1033&mi_00";
    else if (strstr(name, "NV Display Right") || strstr(name, "pid_2033") ||
             (strstr(name, "NV Graphics") && !strstr(name, "MIDI")))
        path = "\\\\?\\usb#vid_15e4&pid_2033&mi_00";
    else if (strstr(name, "NV Control") || strstr(name, "pid_1005"))
        path = "\\\\?\\usb#vid_15e4&pid_1005&mi_00";
    if (path && strlen(path) + 1 <= name_size)
        strcpy(name, path);
}
...
static void port_add(snd_seq_client_info_t* cinfo, snd_seq_port_info_t* pinfo, unsigned int cap, unsigned int type)
{
    ...
    if (cap & SND_SEQ_PORT_CAP_WRITE) {
        ...
        if (!type)
            return;
        if (nv_skip_port(snd_seq_client_info_get_name(cinfo),
                         snd_seq_port_info_get_name(pinfo)))
            return;
        ...
        */
        ...
        nv_rewrite_usb_path(name, sizeof(name));
        len = strlen(name);
        ntdll_umbstowcs( name, len + 1, dest->caps.szPname, ARRAY_SIZE(dest->caps.szPname));
        nv_apply_usb_ids(name, &dest->caps.wMid, &dest->caps.wPid);
        nv_apply_usb_ids(snd_seq_port_info_get_name(pinfo), &dest->caps.wMid, &dest->caps.wPid);
    ...
    if (cap & SND_SEQ_PORT_CAP_READ) {
        ...
        */
        ...
        nv_apply_usb_ids(name, &src->caps.wMid, &src->caps.wPid);
        nv_apply_usb_ids(snd_seq_port_info_get_name(pinfo), &src->caps.wMid, &src->caps.wPid);
        src->state = 0;
