...
static NTSTATUS alsa_get_prop_value(void *args)
{
    ...
    if(IsEqualPropertyKey(*prop, devicepath_key))
    {
        ...
        /* Prefer stable /proc/asound/cardN/usbid (survives hybrid MIDI claim). */
        if(sscanf(name, "plughw:%u,%u", &card, &device) == 2
           || sscanf(name, "hw:%u,%u", &card, &device) == 2){
            FILE *fusbid;
            sprintf(uevent, "/proc/asound/card%u/usbid", card);
            if((fusbid = fopen(uevent, "r"))){
                unsigned int vid = 0, pid = 0;
                if(fscanf(fusbid, "%x:%x", &vid, &pid) == 2 && vid && pid){
                    connection = AudioDeviceConnectionType_USB;
                    vendor_id = (USHORT)vid;
                    product_id = (USHORT)pid;
                }
                fclose(fusbid);
            }
                ...
                if((val = strchr(line, '='))) {
                    ...
                    if(!strcmp(line, "PCI_ID")){
                        ...
                        if(sscanf(val, "%hx/%hx", &vendor_id, &product_id)<2){
                            ...
                            /* do not force Unknown — usbid may already have set USB */
                        }
                        else
                            connection = AudioDeviceConnectionType_USB;
                }
